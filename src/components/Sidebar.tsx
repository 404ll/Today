...
import { ChevronsLeft, Menu, MessageSquare, Plus, Trash2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-200 flex flex-col transition-all">
      <div className="p-4 border-b border-gray-100 flex items-center gap-2 h-16"  onClick={toggleSideBar}>
          <img src="/logo.png" alt="Today" className="w-10 h-10 rounded-full" />
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
        <div className="text-xs font-bold text-gray-400 uppercase tracking-wider px-2 py-2">Your Plans</div>
        {sessions.length === 0 && <div className="text-sm text-gray-400 px-2 italic">No sessions.</div>}
        {sessions.map((session) => (
          ...
          >
            <div className="flex items-center gap-3 overflow-hidden min-w-0">
              <MessageSquare
                size={18}
                className={`flex-shrink-0 ${activeId === session.id ? 'text-blue-500' : 'text-gray-400'}`}
              />
      ...
      <div className="p-4 border-t border-gray-100 text-xs text-gray-400">v0.1 simple</div>
    </div>
  );
};
