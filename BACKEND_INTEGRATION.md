...
### 目标架构
```
前端 (React + Vite)         后端 (Node.js)             数据库
  └─ API Client              └─ REST API                └─ PostgreSQL
  └─ 状态管理 (React Query)   └─ 业务逻辑层              └─ Prisma ORM
  └─ 类型共享 (TypeScript)   └─ AI 服务层
                              └─ 数据访问层
```
...
### 架构特点
- **前端层**：负责 UI 展示和用户交互，通过 API Client 与后端通信
- **后端 API 层**：处理 HTTP 请求，路由分发，请求验证
- **业务逻辑层**：核心业务规则，Session、Todo、Message 的管理
- **AI 服务层**：独立的 AI 服务，处理 OpenAI 调用，待办事项生成
- **数据访问层**：通过 Prisma ORM 访问数据库，封装数据操作
- **数据层**：PostgreSQL 数据库，持久化存储
